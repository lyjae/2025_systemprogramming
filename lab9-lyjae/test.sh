...
if [ "$#" -ne 1 ]; then
  echo "ERROR: Usage: $0 <pipe|webserver|webserver-mt>"
  exit 1
fi

TARGET="$1"

if [[ "$TARGET" != "pipe" && "$TARGET" != "webserver" && "$TARGET" != "webserver-mt" ]]; then
  echo "ERROR: Invalid input '$TARGET'. Only 'pipe', 'webserver', or 'webserver-mt' are allowed."
  exit 1
fi
