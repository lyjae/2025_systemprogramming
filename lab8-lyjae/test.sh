...
if [ "$#" -ne 1 ]; then
  echo "ERROR: Usage: $0 <wc_mt|wc_mt_overlap>"
  exit 1
fi

TARGET="$1"

if [[ "$TARGET" != "wc_mt" && "$TARGET" != "wc_mt_overlap" ]]; then
  echo "ERROR: Invalid input '$TARGET'. Only 'wc_mt' or 'wc_mt_overlap' are allowed."
  exit 1
fi
