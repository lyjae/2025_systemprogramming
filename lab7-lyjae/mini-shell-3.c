...
void expand_variables(char *line) {
    char buffer[MAX_LINE]; int bi = 0;
    for (int i = 0; line[i] != '\0';) {
        if (line[i] == '$') {
            i++;
            char varname[64]; int vi = 0;
            while (isalnum(line[i]) || line[i] == '_') varname[vi++] = line[i++];
            varname[vi] = '\0';
            const char *val = get_var_value(varname);
            for (int j = 0; val[j]; j++) buffer[bi++] = val[j];
        } else buffer[bi++] = line[i++];
    }
    buffer[bi] = '\0';
...
typedef struct { pid_t pid; char command[MAX_LINE]; int stopped; } Job;
Job jobs[MAX_JOBS];
int job_count = 0;
pid_t fg_pid = -1;
void print_job_status(int idx) {
    const char *status = jobs[idx].stopped ? "Stopped" : "Running";
    printf("[%d] %-8s %d %s\n", idx+1, status, jobs[idx].pid, jobs[idx].command);
}
...
int execute_external_command(char **args) {
    ...
    char *clean_args[MAX_ARGS]; int j = 0;
    ...
    if (pid == 0) {
        ...
        if (background) {
            ...
            for (int k = 0; clean_args[k]; k++) {
                strncat(jobs[job_count].command, clean_args[k], sizeof(jobs[job_count].command)-1);
                if (clean_args[k+1]) strncat(jobs[job_count].command, " ", sizeof(jobs[job_count].command)-1);
            }
            job_count++;
...
void execute_command(char **args) {
    ...
    if (strcmp(args[0], "jobs") == 0) {
        for (int i = 0; i < job_count; i++) print_job_status(i);
        return;
    }
    if (strcmp(args[0], "fg") == 0) {
        int idx = -1;
        if (!args[1]) {
            for (int i = job_count-1; i >= 0; i--) if (jobs[i].stopped) { idx = i; break; }
            if (idx < 0) { printf("fg: no current job\n"); return; }
        } else {
            int n = atoi(args[1]);
            if (n < 1 || n > job_count) { printf("fg: job %s not found\n", args[1]); return; }
            idx = n - 1;
            if (!jobs[idx].stopped) { printf("fg: job %d not stopped\n", n); return; }
        }
...
void handle_if_block(char *if_line, FILE *input) {
    ...
    if (!trimmed || strcmp(trimmed, "then") != 0) return;
    char block[MAX_BLOCK_LINES][MAX_LINE];
    int count = 0, found_fi = 0;
    while (fgets(line, sizeof(line), input)) {
        if (is_blank_line(line)) continue;
        char tmp[MAX_LINE]; strncpy(tmp, line, MAX_LINE); tmp[MAX_LINE-1] = '\0';
        char *first_token = strtok(tmp, " \t\n");
        if (first_token && strcmp(first_token, "fi") == 0) { found_fi = 1; break; }
        if (count >= MAX_BLOCK_LINES) return;
        strncpy(block[count], line, MAX_LINE); block[count][MAX_LINE-1] = '\0';
        count++;
    }
    if (!found_fi) return;
    char *args[MAX_ARGS];
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        printf("mini-shell> "); fflush(stdout);
