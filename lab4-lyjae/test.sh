...
if [ "$#" -ne 1 ]; then
  echo "ERROR: Usage: $0 <pw_input|broadcaster|typer>"
  exit 1
fi

TARGET="$1"

if [[ "$TARGET" != "pw_input" && "$TARGET" != "broadcaster" && "$TARGET" != "typer" ]]; then
  echo "ERROR: Invalid input '$TARGET'. Only 'pw_input', 'broadcaster', or 'typer' are allowed."
  exit 1
fi
