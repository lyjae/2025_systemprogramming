...
if [ "$#" -ne 1 ]; then
  echo "ERROR: Usage: $0 <move|editor|snake>"
  exit 1
fi

TARGET="$1"

if [[ "$TARGET" != "move" && "$TARGET" != "editor" && "$TARGET" != "snake" ]]; then
  echo "ERROR: Invalid input '$TARGET'. Only 'move', 'editor', or 'snake' are allowed."
  exit 1
fi
